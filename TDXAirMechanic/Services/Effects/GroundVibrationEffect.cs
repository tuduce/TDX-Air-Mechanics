...
namespace TDXAirMechanic.Services.Effects
{
    public class GroundVibrationEffect : IEffect
    {
        ...
        public void Update(SimVariableData data)
        {
            ...
            if (js == null || _profile?.GroundVibration != true)
                return;
        ...
        private void EnsureOrUpdate(Joystick js, SimVariableData data)
        {
            try
            {
                ...
                bool isConcrete = surface == 0;
                bool isGrass = surface == 1 || surface == 5 || surface == 12;
                bool isAsphalt = surface == 4 || surface == 17;
                ...
                else if (isConcrete)
                {
                    ...
                    var updBg = new EffectParameters
                    {
                        Flags = EffectFlags.ObjectOffsets | EffectFlags.Cartesian,
                        Duration = int.MaxValue,
                        SamplePeriod = 0,
                    ...
                    // Fire jolts only when moving and distance exceeds ~20m
                    if (constantInfo != null && gs > 0.5 && _groundDistanceAccumM >= 20.0)
                    {
                        // consume one stride
                        ...
                        if (_groundJoltEffect == null)
                        {
                            ...
                            epJ.Parameters = new ConstantForce { Magnitude = 1500 };
