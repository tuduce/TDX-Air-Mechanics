...
namespace TDXAirMechanic.Services.Effects
{
    public class GearVibrationEffect : IEffect
    {
        ...
        public void ApplyProfile(AirplaneProfile? profile) { _profile = profile; if (_profile?.GearVibration != true) Reset(); }
        public void Reset() { Remove(); }
        public void Start() { }
        public void Stop() { try { _effect1?.Stop(); } catch { } try { _effect2?.Stop(); } catch { } }
        public void Dispose() { Reset(); }

        public void Update(SimVariableData data)
        {
            var js = GetJoystickSafe(); if (js == null || _profile?.GearVibration != true) return;
            bool gearDown = data.GearPosition >= 0.5; bool onGround = data.OnGround >= 0.5;
            if (!(gearDown && !onGround)) { Remove(); return; }
        ...
        private void EnsureOrUpdate(Joystick js, SimVariableData data)
        {
            try
            {
                ...
                if (_effect1 == null)
                {
                    ...
                    ep1.Parameters = new PeriodicForce { Magnitude = mag1, Offset = 0, Phase = 0, Period = 250000 };
                    _effect1 = new Effect(js, periodicInfo.Guid, ep1); _effect1.Start(1);
                }
                else
                {
                    var update1 = new EffectParameters { Flags = EffectFlags.ObjectOffsets | EffectFlags.Cartesian, Duration = int.MaxValue, SamplePeriod = 0, Gain = 10000, TriggerButton = -1, TriggerRepeatInterval = 0, Parameters = new PeriodicForce { Magnitude = mag1, Offset = 0, Phase = 0, Period = 300000 } };
                    update1.SetAxes(axes, dirs); _effect1.SetParameters(update1, EffectParameterFlags.TypeSpecificParameters | EffectParameterFlags.Start);
                }
                if (_effect2 == null)
                {
                    ...
                    ep2.Parameters = new PeriodicForce { Magnitude = mag2, Offset = 0, Phase = 18000, Period = 22000 };
                    _effect2 = new Effect(js, periodicInfo.Guid, ep2); _effect2.Start(1);
                }
                else
                {
                    var update2 = new EffectParameters { Flags = EffectFlags.ObjectOffsets | EffectFlags.Cartesian, Duration = int.MaxValue, SamplePeriod = 0, Gain = 10000, TriggerButton = -1, TriggerRepeatInterval = 0, Parameters = new PeriodicForce { Magnitude = mag2, Offset = 0, Phase = 18000, Period = 22000 } };
                    update2.SetAxes(axes, dirs); _effect2.SetParameters(update2, EffectParameterFlags.TypeSpecificParameters | EffectParameterFlags.Start);
                }
            }
            catch (Exception ex)
            {
